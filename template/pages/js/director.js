...
fetch();
    ...
    $('#submit').on("click", function () {
        ...
        type = $('#type').val();
        dob = $('#dob').val();
        directedno = $('#directedno').val();
        place = $('#place').val();
        gender = $('#gender').val();
        image = $('#img').val();
        summernote = $('#summernote').val();
        if (fname == "" || type == "" || dob == "" || directedno == "" || place == "" || gender == "" || image == "" || summernote == "") {
            ...
            console.log(fname,dob,directedno,place,gender,image,summernote,type)
              $('#submit').hide();
              $('#loader').show();
...
$(document).on('click', '#actorview', function(){
  ...
  $(document).on('click', '#actor-edit', function(){
      ...
      $.ajax({
        ...
        success:function(data)
        {
            ...
            //      sname1 = $('#type1').val(parsedData[1]);
            var values = parsedData[1];
            $.each(values.split(","),function(i,e){
                  $("#type1 Option[value='" + e + "']").prop("selected",true);
                  console.log(e);
            })
                 dob1 = $('#dob1').val(parsedData[2]);
                 actedno1 = $('#directedno1').val(parsedData[3]);
  ...
  $(document).on('click',"#update",function(){
     ...
     type1 = $('#type1').val();
     dob1 = $('#dob1').val();
     actedno1 = $('#directedno1').val();
     place1 = $('#place1').val();
     gender1 = $('#gender1').val();
     summernote1 = $('#summernote1').val();
     image1 = $('#img1').val();
     if(updateid==""||fname1 =="" ||type1=="" || dob1  =="" ||actedno1=="" ||place1=="" ||gender1=="" ||summernote1==""){
      swal({
            icon: "error",
            title: "All Feilds Required!",
      });
            ...
            $.ajax({
                  ...
                  success: function (data) {
                        if(data == "done"){
                              ...
                              $('#update').show();
                              $('#loader1').hide();   
                              swal("Successfully Updated", "Crew Updated", "success");
                        }
                  }
            })
     }      
      ...
      $.ajax({
        ...
        success:function(data)
        {
             if(data)
             {
                  ...
                  swal("Successfully Deleted", "Crew Deleted", "success");
                  $('#view-data').DataTable().ajax.reload()
             }
        }
      })
